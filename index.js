...
console.clear();
console.log('eror not found index.js...');
...
async function clientstart() {
    ...
    client.ev.on("messages.upsert", async (chatUpdate, msg) => {
        try {
            ...
            if (mek.key.id.startsWith('RyuMa')) return;
            const m = smsg(client, mek, store)
            require("./RyuMa")(client, m, chatUpdate, store)
        } catch (err) {
            console.log(err)
        }
    });
    ...
    client.ev.on('connection.update', (update) => {
        const { konek } = require('./all/connection/connect')
